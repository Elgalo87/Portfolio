window.onload = function(){
  //selecting body element form DOM
    var body = document.querySelector("#calculatorJS")
  ...
  function rows(arr){
    ...
    for (var i = 0; i < arr.length; i ++){
      if (Number(arr[i]) || arr[i] === "." || arr[i] === "0"){
        html += '<div class="btn1"><a href="#" onClick>' + arr[i] + '</a></div>'
      }
      if (isSymbol(arr[i])){
        html += '<div class="btn1 symbol"><a href="#">' + arr[i] + '</a></div>'
      }
      if (arr[i] === "="){
        html += '<div class="btn1 equal symbol"><a href="#">' + arr[i] + '</a></div>'
      }
...
// Calculation formula
  ...
  var listLink = document.querySelectorAll(".btn1")
